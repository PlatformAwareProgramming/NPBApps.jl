...
function update_and_test_face_out_count(f, z, l1, h1, l2, h2, n1, n2)
   ...
   try   
         ...
         #@info "update_and_test_face_out_count --- t=$t --- $(face_out_count[z][f]) == $((n1[z]-2)*(n2[z]-2)*5)"
         if t
...
function deposit_face(z, l1, h1, l2, h2, buffer, _::Val{1})
   view(faces_out[z][1], 1:5, l1:h1, l2:h2) .= buffer 
   lock(lk1)
   try
...
function deposit_face(z, l1, h1, l2, h2, buffer, _::Val{2})
   view(faces_out[z][2], 1:5, l1:h1, l2:h2) .= buffer 
   lock(lk1)
   try
...
function deposit_face(z, l1, h1, l2, h2, buffer, _::Val{3})
   view(faces_out[z][3], 1:5, l1:h1, l2:h2) .= buffer 
   lock(lk1)
   try
...
function deposit_face(z, l1, h1, l2, h2, buffer, _::Val{4})
   view(faces_out[z][4], 1:5, l1:h1, l2:h2) .= buffer 
   lock(lk1)
   try
...
function collect_face(z, l1, h1, l2, h2, _::Val{1})
   ...
   try
      ...
      r = view(faces_in[z][1], 1:5, l1:h1, l2:h2)    
...
function collect_face(z, l1, h1, l2, h2, _::Val{2})
   ...
   try
      ...
      r = view(faces_in[z][2], 1:5, l1:h1, l2:h2) 
...
function collect_face(z, l1, h1, l2, h2, _::Val{3})
   ...
   try
      ...
      r = view(faces_in[z][3], 1:5, l1:h1, l2:h2) 
...
function collect_face(z, l1, h1, l2, h2, _::Val{4})
   ...
   try
      ...
      r = view(faces_in[z][4], 1:5, l1:h1, l2:h2) 
...
function go_cluster(clusters, niter, dt, ratio, x_zones, y_zones, gx_size, gy_size, gz_size, nxmax, nx_, ny_, nz_, proc_num_zones_all, x_size, y_size, zone_proc_id_, proc_zone_id_, iz_west_, iz_east_, iz_south_, iz_north_, itimer, npb_verbose)
   ...
   for iz = 1:proc_num_zones
      ...
      faces_out[iz][1] = Array{Float64}(undef, 5, ny[iz], gz_size) 
      faces_out[iz][2] = Array{Float64}(undef, 5, ny[iz], gz_size) 
      faces_out[iz][3] = Array{Float64}(undef, 5, nx[iz], gz_size) 
      faces_out[iz][4] = Array{Float64}(undef, 5, nx[iz], gz_size) 
   end

   global faces_in = Array{Array{Array{Float64}}}(undef, proc_num_zones)
   for iz = 1:proc_num_zones
      faces_in[iz] = Array{Array{Float64}}(undef, 4)
      faces_in[iz][1] = Array{Float64}(undef, 5, ny[iz], gz_size)
      faces_in[iz][2] = Array{Float64}(undef, 5, ny[iz], gz_size)
      faces_in[iz][3] = Array{Float64}(undef, 5, nx[iz], gz_size)
      faces_in[iz][4] = Array{Float64}(undef, 5, nx[iz], gz_size)
   end
   ...
   BT.go_node(clusterid, clusters, niter, dt, ratio, x_zones, y_zones, gx_size, gy_size, gz_size, nxmax, nx, ny, nz, proc_num_zones, itimer, npb_verbose) 
end
...
function go_node(cluster_id, no_nodes, niter, dt, ratio, x_zones, y_zones, gx_size, gy_size, gz_size, nxmax, nx, ny, nz, proc_num_zones, itimer, npb_verbose)
   @everywhere workers() BT.perform($cluster_id, $no_nodes, $niter, $dt, $ratio, $x_zones, $y_zones, $gx_size, $gy_size, $gz_size, $nxmax, $nx, $ny, $nz, $proc_num_zones, $itimer, $npb_verbose) 
end
