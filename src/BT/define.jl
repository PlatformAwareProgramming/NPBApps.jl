...
#   for c = 1:ncells
#       @info "++++ $clusterid/$node: zone=$z ncells=$ncells c=$c cell_size[$z][1,$c]=$(cell_size[z][1,c]) cell_size[$z][2,$c]=$(cell_size[z][2,c]) cell_size[$z][3,$c]=$(cell_size[z][3,c]) dim=$dim"
#   end
   ...
   #@info "**** $clusterid/$node: zone=$z start_send_west[$z]=$(start_send_west[z]) start_send_east[$z]=$(start_send_east[z]) start_send_south[$z]=$(start_send_south[z])  start_send_north[$z]=$(start_send_north[z]) start_send_bottom[$z]=$(start_send_bottom[z]) start_send_top[$z]=$(start_send_top[z])"
