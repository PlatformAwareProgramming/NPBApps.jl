...
function perform_collect_face(z, l1, h1, l2, h2, f, n1, n2)
   ...
   # @info "$clusterid: collect_face($f) BEGIN 1 --- zone=$zone z=$z $l1/$h1/$l2/$h2"
      ...
      try
         ...
         # @info "$clusterid: collect_face($f) PASSED --- zone=$zone z=$z"
      finally
         unlock(face_in_collect[z][f])
      end
...
function go_cluster(clusters, niter, dt, ratio, x_zones, y_zones, gx_size, gy_size, gz_size, problem_size, nxmax, nx_, ny_, nz_, proc_num_zones_all, x_size, y_size, zone_proc_id_, proc_zone_id_all, iz_west_, iz_east_, iz_south_, iz_north_, itimer, npb_verbose)
   ...
   for iz = 1:proc_num_zones
      ...
      face_out[iz][1] = Array{Float32}(undef, 5, ny[zone], gz_size) 
      face_out[iz][2] = Array{Float32}(undef, 5, ny[zone], gz_size) 
      face_out[iz][3] = Array{Float32}(undef, 5, nx[zone], gz_size) 
      face_out[iz][4] = Array{Float32}(undef, 5, nx[zone], gz_size) 
   end
   ...
   for iz = 1:proc_num_zones
      ...
      face_in[iz][1] = Array{Float32}(undef, 5, ny[zone], gz_size)
      face_in[iz][2] = Array{Float32}(undef, 5, ny[zone], gz_size)
      face_in[iz][3] = Array{Float32}(undef, 5, nx[zone], gz_size)
      face_in[iz][4] = Array{Float32}(undef, 5, nx[zone], gz_size)
   end
   ...
   BT.go_node(clusterid, clusters, niter, dt, ratio, x_zones, y_zones, gx_size, gy_size, gz_size, problem_size, nxmax, nx, ny, nz, proc_num_zones, proc_zone_id, zone_proc_id, iz_west, iz_east, iz_north, iz_south, itimer, npb_verbose) 
end

function go_node(clusterid, no_nodes, niter, dt, ratio, x_zones, y_zones, gx_size, gy_size, gz_size, problem_size, nxmax, nx, ny, nz, proc_num_zones, proc_zone_id, zone_proc_id, iz_west, iz_east, iz_north, iz_south, itimer, npb_verbose)
   @everywhere workers() BT.perform($clusterid, $no_nodes, $niter, $dt, $ratio, $x_zones, $y_zones, $gx_size, $gy_size, $gz_size, $problem_size, $nxmax, $nx, $ny, $nz, $proc_num_zones, $proc_zone_id, $zone_proc_id, $iz_west, $iz_east, $iz_north, $iz_south, $itimer, $npb_verbose)  
end
