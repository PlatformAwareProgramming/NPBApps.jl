...
const tsum = Array{Float32}(undef, t_last+2)
const tming = Array{Float32}(undef, t_last+2)
const tmaxg = Array{Float32}(undef, t_last+2)
const trecs = Array{Float32}(undef, t_last+2)
...
function go(class::CLASS; itimer=0, npb_verbose=0, zone_mapping = nothing )
   ...
   global tmax_all = DataFlowVector(Array{Float32}(undef, num_clusters))
   global timers_all = DataFlowVector(Array{Tuple{Vector{Float32}, Vector{Float32}, Vector{Float32}}}(undef, num_clusters))
   ...
   @everywhere workers() BT.go_cluster($clusters, $niter, $dt, $ratio, $x_zones, $y_zones, $gx_size, $gy_size, $gz_size, $problem_size, $nxmax, $nx, $ny, $nz, $proc_num_zones, $x_size, $y_size, $zone_proc_id, $proc_zone_id, $iz_west, $iz_east, $iz_south, $iz_north, $itimer, $npb_verbose)       
