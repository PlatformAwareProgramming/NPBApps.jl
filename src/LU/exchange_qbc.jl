...
function exch_qbc(proc_num_zones, zone_proc_id, proc_zone_id, u, row, col, west, east, north, south, west2, east2, north2, south2, nx, ny, nz, timeron, ist, iend, jst, jend, iz_west, iz_east, iz_south, iz_north, comm_exch, buf_exch_w_out, buf_exch_e_out, buf_exch_n_out, buf_exch_s_out, buf_exch_w_in, buf_exch_e_in, buf_exch_n_in, buf_exch_s_in, requests)
   ...
   for iz = 1:proc_num_zones
      ...
      #@info "$clusterid/$node --- REQUESTS COUNTER --- $(r[]) $(length(requests)) $tag_west $tag_east $tag_north $tag_south $(west[iz]) $(east[iz]) $(north[iz]) $(south[iz])"

      requests[r[]+0] = tag_west  > 0 && west[iz] == -1 ? (rr = MPI.Irecv!(buf_exch_w_in[iz], comm_exch; source = west2[iz],  tag = tag_west);#=@info "$clusterid/$node: zone=$zone - RECV W src=$(west2[iz]) $tag_west -- $rr";=# rr) : (MPI.REQUEST_NULL)
      requests[r[]+1] = tag_east  > 0 && east[iz] == -1 ? (rr = MPI.Irecv!(buf_exch_e_in[iz], comm_exch; source = east2[iz],  tag = tag_east);#=@info "$clusterid/$node: zone=$zone - RECV E src=$(east2[iz]) $tag_east -- $rr";=# rr) : (MPI.REQUEST_NULL)
      requests[r[]+2] = tag_north > 0 && north[iz] == -1 ? (rr = MPI.Irecv!(buf_exch_n_in[iz], comm_exch; source = north2[iz], tag = tag_north);#=@info "$clusterid/$node: zone=$zone - RECV N src=$(north2[iz]) $tag_north -- $rr";=# rr) : (MPI.REQUEST_NULL)
      requests[r[]+3] = tag_south > 0 && south[iz] == -1 ? (rr = MPI.Irecv!(buf_exch_s_in[iz], comm_exch; source = south2[iz], tag = tag_south);#=@info "$clusterid/$node: zone=$zone - RECV S src=$(south2[iz]) $tag_south -- $rr";=# rr) : (MPI.REQUEST_NULL)
...
#---------------------------------------------------------------------
      ...
      if east == -1 && east_zone != -1
         ...
         if zone_cluster != clusterid
            ...
            kst = 1
            kend = nz-2
            #@info "$clusterid/$node: copy faces send east - BEGIN - z=$z zone=$zone  ist=$ist  iend=$iend  kst=$kst  kend=$kend ---> $u_face" 
            ...
            tag_east = from_w + z 
            buf_exch_e_out .= reshape(u_face, 5*nx*(nz-2))
      ...
      if west == -1 && west_zone != -1
         ...
         if zone_cluster != clusterid
            ...
            kst = 1
            kend = nz-2
            #@info "$clusterid/$node: copy faces send west - BEGIN - z=$z zone=$zone  ist=$ist  iend=$iend  kst=$kst  kend=$kend ---> $u_face" 
            ...
            tag_west = from_e + z
            buf_exch_w_out .= reshape(u_face, 5*nx*(nz-2))
...
#---------------------------------------------------------------------
     ...
     if south == -1 && south_zone != -1
         u_face = view(u, 1:5, nx-1, 1:ny, 1:nz-2)
         ...
         if zone_cluster != clusterid
            ...
            kst = 1
            kend = nz-2
            #@info "$clusterid/$node: copy faces send south - BEGIN - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend ---> $u_face" 
            ...
            tag_south = from_n + z
            buf_exch_s_out .= reshape(u_face, 5*(ny)*(nz-2))
...
#---------------------------------------------------------------------
      if north == -1 && north_zone != -1
         u_face = view(u, 1:5, 2, 1:ny, 1:nz-2)
         ...
         if zone_cluster != clusterid
            ...
            kst = 1
            kend = nz-2
            #@info "$clusterid/$node: copy faces send north - BEGIN - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend ---> $u_face" 
            ...
            tag_north = from_s + z
            buf_exch_n_out .= reshape(u_face, 5*(ny)*(nz-2))
...
#---------------------------------------------------------------------
      ...
      if south == -1 && south_zone != -1
         ...
         if zone_cluster != clusterid
            ...
            kst = 1
            kend = nz-2
            #@info "$clusterid/$node: copy faces recv south - BEGIN 1 - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend" 
            u_face = remotecall(collect_face, 1, z, jst, jend, kst, kend, Val(3); role=:worker)
            #@info "$clusterid/$node: copy faces recv south - BEGIN 2 - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend" 
            u_face_fetch = fetch(u_face; role=:worker)
            #@info "$clusterid/$node: copy faces recv south - BEGIN 3 - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend" 
            view(u, 1:5, nx, 1:ny, 1:nz-2) .= u_face_fetch
            #@info "$clusterid/$node: copy faces recv south - END - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend ---> $u_face_fetch" 
         else
            u_face = reshape(buf_exch_s_in, 5, ny, nz-2)
            #@info "$clusterid/$node: z=$z zone=$zone --- RECV FROM south (target_zone=$south_zone) ny=$ny ---> $u_face"
            view(u, 1:5, nx, 1:ny, 1:nz-2) .= u_face
         end
      end
      ...
      if north == -1 && north_zone != -1
         ...
         if zone_cluster != clusterid
            ...
            kst = 1
            kend = nz-2
            #@info "$clusterid/$node: copy faces recv north - BEGIN 1 - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend" 
            u_face = remotecall(collect_face, 1, z, jst, jend, kst, kend, Val(4); role=:worker)
            #@info "$clusterid/$node: copy faces recv north - BEGIN 2 - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend" 
            u_face_fetch = fetch(u_face; role=:worker)
            #@info "$clusterid/$node: copy faces recv north - BEGIN 3 - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend" 
            view(u, 1:5, 1, 1:ny, 1:nz-2) .= u_face_fetch
            #@info "$clusterid/$node: copy faces recv north - END - z=$z zone=$zone  jst=$jst  jend=$jend  kst=$kst  kend=$kend ---> $u_face_fetch" 
         else
            u_face = reshape(buf_exch_n_in, 5, ny, nz-2)
            #@info "$clusterid/$node: z=$z zone=$zone --- RECV FROM north (target_zone=$north_zone) ny=$ny ---> $u_face"
            view(u, 1:5, 1, 1:ny, 1:nz-2) .= u_face
         end
      end
      ...
      if west == -1 && west_zone != -1
         ...
         if zone_cluster != clusterid
            ...
            #@info "$clusterid/$node: copy faces recv west - END - z=$z zone=$zone  ist=$ist  iend=$iend  kst=$kst  kend=$kend ---> $u_face_fetch" 
         else
            u_face = reshape(buf_exch_w_in, 5, nx, nz-2)
            #@info "$clusterid/$node: z=$z zone=$zone --- RECV FROM west (target_zone=$west_zone) ---> $u_face"
      ...
      if east == -1 && east_zone != -1
         ...
         if zone_cluster != clusterid
            ...
            kst = 1
            kend = nz-2
            #@info "$clusterid/$node: copy faces recv east - BEGIN 1 - z=$z zone=$zone  ist=$ist  iend=$iend  kst=$kst  kend=$kend" 
            u_face = remotecall(collect_face, 1, z, ist, iend, kst, kend, Val(2); role=:worker)
            #@info "$clusterid/$node: copy faces recv east - BEGIN 2 - z=$z zone=$zone  ist=$ist  iend=$iend  kst=$kst  kend=$kend" 
            u_face_fetch = fetch(u_face; role=:worker)
            #@info "$clusterid/$node: copy faces recv east - BEGIN 3 - z=$z zone=$zone  ist=$ist  iend=$iend  kst=$kst  kend=$kend" 
            view(u, 1:5, 1:nx, ny, 1:nz-2) .= u_face_fetch
            #@info "$clusterid/$node: copy faces recv east - END - z=$z zone=$zone  ist=$ist  iend=$iend  kst=$kst  kend=$kend ---> $u_face_fetch" 
         else            
            u_face = reshape(buf_exch_e_in, 5, nx, nz-2)
