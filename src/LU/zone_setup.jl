function zone_setup(x_zones, y_zones, gx_size, gy_size, gz_size, nx, nxmax, ny, nz, nx1, ratio, npb_verbose)
...
#        compute essentially equal sized zone dimensions
       ...
       for j = 1:y_zones
         for i = 1:x_zones
           ...
           # nx is the subdomain size partitioned from nx1
           #nx[zone_no] = subdomain(nx1[zone_no], ny[zone_no], nz[zone_no])
           nxmax[zone_no] = nx[zone_no] + 1 - mod(nx[zone_no], 2)
            ...
            if row > 1
                iz_north[zone_no] = zone_no - 1
            else
                iz_north[zone_no] = col * x_zones
            end

            if row < x_zones
                iz_south[zone_no] = zone_no + 1
            else
                iz_south[zone_no] = zone_no - x_zones + 1
            end

            if col > 1
                iz_west[zone_no] = zone_no - x_zones
            else
                iz_west[zone_no] = zone_no + y_zones * (x_zones - 1)  # -1
            end

            if col < y_zones
                iz_east[zone_no] = zone_no + x_zones
            else
                iz_east[zone_no] = zone_no - y_zones * (x_zones - 1)   # -1
            end           
