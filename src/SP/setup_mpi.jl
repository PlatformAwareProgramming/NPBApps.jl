...
function setup_mpi(proc_num_zones)
...
#      end
      comm_solve_ =  MPI.Comm_dup(comm_setup)
      comm_rhs_ =  MPI.Comm_dup(comm_setup)
      for iz = 1:proc_num_zones
          comm_solve[iz] = comm_solve_ # MPI.Comm_dup(comm_setup)
          comm_rhs[iz] = comm_rhs_ # MPI.Comm_dup(comm_setup)
      end
...
#---------------------------------------------------------------------
      ...
      # ASSUMING THAT ALL CLUSTERS ARE ACTIVE.
      #TODO: processos-cluster devem conversar para saber quantos estão ativos (num_clusters)
      #global num_clusters = no_nodes

      #mpi_comm_size(comm_setup, num_clusters, ierror)
      #mpi_comm_rank(comm_setup, clusterid, ierror)
      #if no_nodes != num_clusters
      #   if (clusterid == root_global && node == root) 
      #      @printf(stdout, "Warning: Requested %6i MPI processes exceeds the number of zones %5i\nThe value %5i is used for benchmarking\n", no_nodes, max_zones, num_clusters) 
      #   end
      #end
#
#
