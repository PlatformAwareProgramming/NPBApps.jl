...
function go_cluster(clusters, niter, dt, ratio, x_zones, y_zones, gx_size, gy_size, gz_size, problem_size, nxmax, nx_, ny_, nz_, proc_num_zones_all, x_size, y_size, zone_proc_id_, proc_zone_id_all, iz_west_, iz_east_, iz_south_, iz_north_, itimer, npb_verbose)
   ...
   @info "myid is $(Distributed.myid(role=:worker)) --- $(minimum(Distributed.workers(role=:workers)))"
   ...
   for iz = 1:proc_num_zones
      ...
      face_out[iz][1] = Array{FloatType}(undef, ny[zone], gz_size, 5) 
      face_out[iz][2] = Array{FloatType}(undef, ny[zone], gz_size, 5) 
      face_out[iz][3] = Array{FloatType}(undef, nx[zone], gz_size, 5) 
      face_out[iz][4] = Array{FloatType}(undef, nx[zone], gz_size, 5) 
   end
   ...
   for iz = 1:proc_num_zones
      ...
      face_in[iz][1] = Array{FloatType}(undef, ny[zone], gz_size, 5)
      face_in[iz][2] = Array{FloatType}(undef, ny[zone], gz_size, 5)
      face_in[iz][3] = Array{FloatType}(undef, nx[zone], gz_size, 5)
      face_in[iz][4] = Array{FloatType}(undef, nx[zone], gz_size, 5)
   end
