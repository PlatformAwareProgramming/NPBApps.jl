...
function verify(class, dt, no_time_steps)
        ...
        xcrref = Array{Float32}(undef, 5)
        xceref = Array{Float32}(undef, 5)
        xcrdif = Array{Float32}(undef, 5)
        xcedif = Array{Float32}(undef, 5)
...
function verify(dt, ss, sr, b_size, 
        ...
        xce = Array{Float32}(undef, 5)
        xcr = Array{Float32}(undef, 5)
        xce_sub = Array{Float32}(undef, 5)
        xcr_sub = Array{Float32}(undef, 5)
